...
test('merge static pipelines', async () => {
  ...
  const forks = pipelines.map((pipeline) => source.fork().through(pipeline))

  const results = await xs(forks).merge().toArray()
...
test('merge dynamic pipelines', async () => {
  ...
  // source.start()
  const pipelines = await xs([fork1, fork2]).merge().toArray()
  // Exstream 0.19 used values() here.

  expect(pipelines).toHaveLength(2)

  const forks = pipelines.map((pipeline) => xs(singleInput).fork().through(pipeline))
...
test('merge static pipelines - highland', async () => {
  ...
  const results = await __(forks).merge().collect().toPromise(Promise)
  expect(results).toEqual([1, 1, 2, 2, 3, 3])
})