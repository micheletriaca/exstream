...
test('drop-oldest retains the newest bounded values', () => {
...
test('drop-newest retains the oldest bounded values', () => {
...
test('a zero buffer limit drops every best-effort value', () => {
...
test('observer overflow aborts only the observer', () => {
  ...
  expect(sibling.values()).toEqual([1, 2, 3])
})

test('an observer destroyed early remains detached from source lifecycle', () => {
  const source = _([1, 2, 3])
